...
class AllSync {
  ...
  async syncAllToNotion() {
    try {
      console.log(
        "🚀 Synchronisation complète vers Notion (PRD + Features)...",
      );
      ...
      if (prdSuccess && featuresSuccess) {
        console.log(
          "✅ Synchronisation complète vers Notion terminée avec succès !",
        );
        return true;
      } else {
      ...
      console.error(
        "❌ Erreur lors de la synchronisation complète:",
        error.message,
      );
      return false;
    }
  ...
  async syncAllFromNotion() {
    try {
      console.log(
        "🚀 Synchronisation complète depuis Notion (PRD + Features)...",
      );
      console.log("=".repeat(60));
      ...
      if (prdSuccess && featuresSuccess) {
        console.log(
          "✅ Synchronisation complète depuis Notion terminée avec succès !",
        );
        return true;
      } else {
        console.log("⚠️ Synchronisation terminée avec des erreurs");
...
if (require.main === module) {
  ...
  if (args.includes("--to-notion")) {
    ...
    console.log("Usage:");
    console.log(
      "  node sync-all.js --to-notion     # Synchronise PRD + Features vers Notion",
    );
    console.log(
      "  node sync-all.js --from-notion   # Synchronise PRD + Features depuis Notion",
    );
  }
}
