...
class FeaturesSync {
  ...
  async syncFeaturesToNotion() {
    try {
      ...
      for (const file of mdFiles) {
        ...
        await this.featuresClient.createOrUpdateFeature(featureName, markdownContent, frontMatter);
      }
      
      console.log('✅ Synchronisation des features vers Notion terminée !');
      return true;
    } catch (error) {
      console.error('❌ Erreur de synchronisation des features:', error.message);
      return false;
    }
  }
  ...
  async syncFeaturesFromNotion() {
    try {
      console.log('🚀 Début de la synchronisation des features depuis Notion...');
      ...
      console.log(`📄 ${features.length} feature(s) trouvée(s) dans Notion pour ce projet`);
      ...
      for (const feature of features) {
        ...
        const fullContent = this.generateFileContent(frontMatter, markdownContent);
      ...
      console.error('❌ Erreur de synchronisation depuis Notion:', error.message);
      return false;
    }
  }
...
if (require.main === module) {
  ...
  if (args.includes('--to-notion')) {
    ...
    console.log('Usage:');
    console.log('  node features-sync.js --to-notion     # Synchronise vers Notion');
    console.log('  node features-sync.js --from-notion   # Synchronise depuis Notion');
  }
}
