...
class NotionSync {
  ...
  async syncToNotion() {
    try {
      ...
      const { frontMatter, markdownContent } =
        this.parseFrontMatter(prdContent);

      // Envoyer vers la database Notion avec les propriétés
      await this.notionClient.updateProjectContent(
        markdownContent,
        frontMatter,
      );
      ...
      if (error.code === "ENOENT") {
        console.error(
          '❌ Fichier PRD.md introuvable. Exécutez d\'abord "npm run init-prd"',
        );
      } else {
  ...
  async syncFromNotion() {
    try {
      console.log(
        "🚀 Début de la synchronisation depuis la database Notion...",
      );
  ...
  blocksToMarkdown(blocks, depth = 0, listType = null, numIndex = 1) {
    ...
    for (let i = 0; i < blocks.length; i++) {
      ...
      switch (block.type) {
        ...
        case "paragraph": {
          const paragraphText = this.richTextToMarkdown(
        ...
        case "bulleted_list_item":
          ...
          if (
            ...
            block.bulleted_list_item.children.length > 0
          ) {
            markdown += this.blocksToMarkdown(
              block.bulleted_list_item.children,
              depth + 1,
              "bulleted",
            );
        ...
        case "numbered_list_item": {
          ...
          if (
            ...
            block.numbered_list_item.children.length > 0
          ) {
            markdown += this.blocksToMarkdown(
              block.numbered_list_item.children,
              depth + 1,
